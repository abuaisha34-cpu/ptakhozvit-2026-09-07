...
function createNeonSql(): Promise<Sql> {
  globalRef.__pgSqlPromise__ ??= (async () => {
    ...
    globalRef.__pgPool__ ??= new Pool({ connectionString: databaseUrl, max: 4 });
  ...
  try {
    ...
    for (const { name, path } of pendingMigrations(Object.keys(migrations), applied)) {
      try {
        ...
        if (code === "23505") continue; // concurrent worker already recorded this file
        ...
        try {
          ...
          /* still record so one bad file cannot 500 the site on every request */
        }
      }
    }
  } catch (err) {
