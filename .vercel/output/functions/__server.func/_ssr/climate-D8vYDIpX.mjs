import { i as humidityFromNorms, t as DEFAULT_NORMS } from "./handbook-OalI0BW1.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/climate-D8vYDIpX.js
var KIND_LABEL = {
	clear: "Ясно",
...
function humidityTarget(ageDays, norms = DEFAULT_NORMS) {
	return humidityFromNorms(ageDays, norms);
}
function ageBandLabel(ageDays) {
...
function worst(items) {
	return items.reduce((acc, it) => RANK[it.severity] > RANK[acc] ? it.severity : acc, "ok");
}
function push(items, item) {
	if (item) items.push(item);
}
function buildClimateAdvice(input) {
	const age = Math.max(0, Math.round(input.ageDays));
	const tMin = input.houseTempMin;
	const tMax = input.houseTempMax;
	const rh = input.humidityMin != null && input.humidityMax != null ? {
		min: input.humidityMin,
		max: input.humidityMax
	...
	const finish = age >= 28;
	const items = [];
	if (chicks && out.tempMin < 12) push(items, {
	...
	else if (nightGap >= 12) push(items, {
	...
	if (out.tempMax + 1.5 >= tMin && chicks) push(items, {
		severity: "watch",
	...
	if (!chicks && out.tempMax > tMax + 2) {
		...
		push(items, {
			...
			detail: heat ? `Надворі до ${fmt(out.tempMax)} °C при нормі ${fmt(tMin)}–${fmt(tMax)} °C на ${age} добу. Тунельна вентиляція, швидкість повітря на рівні птаха, додаткові ніпелі. Не годуйте в пік спеки.` : `Максимум ${fmt(out.tempMax)} °C vs норма залу до ${fmt(tMax)} °C. Збільшіть повітрообмін удень, стежте за розльотом птаха від стін і поїлок.`
		});
	}
	if (finish && out.tempMin > tMax) push(items, {
		severity: "warn",
	...
	if (out.windMaxMs >= 8) push(items, {
		severity: chicks ? "warn" : "watch",
		title: "Сильний вітер",
		detail: out.windMaxMs >= 12 ? `Пориви до ${fmt(out.windMaxMs)} м/с. Перевірте штори, припливи й підпір. На брудингу не ловіть «щілину» на рівні підлоги.` : `Вітер до ${fmt(out.windMaxMs)} м/с. Холодний вітер виносить тепло через огородження — додайте потужність опалення, якщо ніч холодніша за ${fmt(tMin)} °C.`
	});
	if (out.precipMm >= 1 || out.kind === "rain" || out.kind === "snow" || out.kind === "thunder") push(items, {
		severity: out.kind === "thunder" ? "watch" : young ? "watch" : "ok",
		title: out.kind === "thunder" ? "Гроза" : out.kind === "snow" ? "Сніг / мокрий холод" : "Волога погода",
		detail: out.kind === "thunder" ? "Резерв живлення вентиляторів і брудера. Після зливи не відкривайте приплив на повну — вологе повітря без підігріву сяде на підстилку." : young ? `Опади ${fmt(out.precipMm, 1)} мм. Вологе повітря + курчата = мокра підстилка і падіж від переохолодження. Підігрівайте приплив, перевірте ніпелі, підстилку тримайте сухою.` : `Опади ${fmt(out.precipMm, 1)} мм. Слідкуйте за вологістю підстилки і аміаком; не переливайте воду.`
	});
	if (out.humidityMean != null && out.humidityMean >= 80 && out.tempMax >= 26 && !chicks) push(items, {
		severity: "warn",
	...
	if (young && out.humidityMean != null && out.humidityMean < 40) push(items, {
		severity: "watch",
	...
	if (indoorMin != null && indoorMin < tMin - 1.5) push(items, {
	...
	if (indoorMax != null && indoorMax > tMax + 1.5) push(items, {
	...
	if (indoorRh != null && (indoorRh < rh.min || indoorRh > rh.max)) push(items, {
		severity: (indoorRh < rh.min - 5 || indoorRh > rh.max + 5) && young ? "warn" : "watch",
		title: indoorRh > rh.max ? "Вологість у залі висока" : "Вологість у залі низька",
		detail: indoorRh > rh.max ? `${fmt(indoorRh)}% при нормі ${rh.min}–${rh.max}%. Ризик мокрої підстилки й аміаку — підігрів + короткі цикли вентиляції, ніпелі без протікання.` : `${fmt(indoorRh)}% при нормі ${rh.min}–${rh.max}%. Зволожте повітря (туман, вода на бетон поза брудером), не охолоджуючи курча.`
	});
	...
	return {
		severity,
		headline: headlineFor(severity, chicks, nightGap, out, tMin, tMax, age),
		...
		items: items.filter((it) => it.severity !== "ok").length ? items.filter((it) => it.severity !== "ok") : items.slice(0, 1)
	};
}
function headlineFor(severity, chicks, nightGap, out, tMin, tMax, age) {
	if (severity === "critical" && chicks) return `Курчата: надворі ${fmt(out.tempMin)}–${fmt(out.tempMax)} °C, у залі треба ${fmt(tMin)}–${fmt(tMax)} °C — опалення і без протягів`;
	if (out.tempMax >= 32 && age >= 21) return `Спека ${fmt(out.tempMax)} °C на ${age} добу — максимум повітря і вода`;
	if (nightGap >= 12) return `Ніч ${fmt(out.tempMin)} °C, норма залу від ${fmt(tMin)} °C — тримайте тепло, вентиляція мінімальна`;
	if (out.tempMax > tMax + 2) return `День ${fmt(out.tempMax)} °C вище за норму залу ${fmt(tMax)} °C — додайте повітрообмін`;
	if (out.kind === "rain" || out.kind === "snow") return `${out.label}, ${fmt(out.tempMin)}–${fmt(out.tempMax)} °C — бережіть підстилку від вологи`;
	return `Надворі ${fmt(out.tempMin)}–${fmt(out.tempMax)} °C · у пташнику тримайте ${fmt(tMin)}–${fmt(tMax)} °C`;
}
function fmt(n, digits = 0) {
	const f = 10 ** digits;
	return (Math.round(n * f) / f).toLocaleString("uk-UA", {
...
//#endregion
export { weatherKindLabel as n, buildClimateAdvice as t };
